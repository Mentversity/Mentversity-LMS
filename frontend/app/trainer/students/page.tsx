...
import { Loader2, BookOpen, ChevronLeft, ChevronRight, Badge } from 'lucide-react';
...
export default function AdminStudentPage() {
  ...
  // Fetch students (always by course)
  useEffect(() => {
    const fetchStudentsData = async () => {
      ...
      setError(null);
      try {
        const response = await studentsApi.getByCourseId(
          selectedCourseId,
          currentPage,
          studentsPerPage
        );
    ...
    fetchStudentsData();
  }, [currentPage, studentsPerPage, selectedCourseId]);

  // Reset page on filter change
  useEffect(() => {
    setCurrentPage(1);
  }, [selectedCourseId, studentsPerPage]);
  ...
  if (isLoading && students.length === 0) {
    return (
      <div
        className={`${inter.className} flex items-center justify-center h-screen bg-gray-50 text-gray-800`}
      >
  ...
  if (error) {
    return (
      <div
        className={`${inter.className} min-h-screen flex flex-col items-center justify-center text-center bg-gray-50 text-gray-700 p-6`}
      >
        <p className="text-lg font-medium">Error loading students</p>
        <p className="text-sm text-gray-500 mb-4">{error}</p>
        <Button
          onClick={() => window.location.reload()}
          className="bg-[#00404a] text-white rounded-full px-6 py-2 shadow-md hover:bg-[#04b895]"
        >
          Retry
        </Button>
      </div>
  ...
  return (
    <div
      className={`${inter.className} space-y-6 p-6 md:p-8 lg:p-10 bg-gray-50 min-h-screen text-gray-900`}
    >
          ...
          <CardContent>
            ...
            {courses.length === 0 && (
              <p className="text-sm text-gray-500 mt-2">
                No courses available to filter.
              </p>
            )}
          </CardContent>
        </Card>
      ...
      <Card className="rounded-xl shadow-lg border border-gray-200">
        ...
        <CardContent className="p-0">
          {(!selectedCourseId || students.length === 0) && !isLoading ? (
                  ...
                  {students.map((student) => (
                    <tr key={student._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 capitalize">
                        <Badge className="bg-[#D3F5E7] text-[#00404a] font-medium border border-[#00404a] text-xs">
                          {student.role}
                        </Badge>
                      </td>
                    </tr>
      ...
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row justify-between items-center mt-6 p-4 bg-white rounded-xl shadow-md border border-gray-200">
          <div className="text-sm text-gray-700 mb-4 sm:mb-0">
            Page {currentPage} of {totalPages} (Showing {students.length} of{' '}
